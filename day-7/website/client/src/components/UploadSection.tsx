...
export function UploadSection({ onResult }: { onResult: (res: { prediction: string, features: Feature[] } | null) => void }) {
  ...
  const analyzeImage = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/predict_restoration', {
        method: 'POST',
        body: formData,
      });
